...
class FedNHPlusClient(FedUHClient):
...
class FedNHPlusServer(FedUHServer):
    def __init__(self, server_config, clients_dict, exclude, **kwargs):
        ...
        if len(self.exclude_layer_keys) > 0:
            print(f"FedNHPlusServer: the following keys will not be aggregated:\n ", self.exclude_layer_keys)
    ...
    def aggregate(self, client_uploads, round):
        ...
        with torch.no_grad():
            for idx, (client_state_dict, prototype_dict) in enumerate(client_uploads):
                if self.server_config['FedNHPlus_server_adv_prototype_agg'] == False:
            ...
            weight = self.server_config['FedNHPlus_smoothing']
